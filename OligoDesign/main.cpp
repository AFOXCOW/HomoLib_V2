...
int FindCutSite(vector<string> Aligned_Sequences, int min_length, int max_length,int high_homo_length, vector<vector<double>> similar_table,vector<int> &cut,vector<int> &reversecut)
{	//[cut[0],cut[1])
	...
	//start from last forward cut site.
	int start_minLen, end_maxLen;
	int start_cut, end_cut;
	score = 0;
	best_score = 0;
	best_site = -1;
	start_minLen = Find_length_index_backward_all(Aligned_Sequences,Aligned_Sequences[0].size() - 1,min_length);
	end_maxLen = Find_length_index_backward_one(Aligned_Sequences, Aligned_Sequences[0].size() - 1, max_length);
	start_cut = cut[cut.size() - 1] - 5; //-5bp,for not near to forward cut site.
	end_cut = cut[cut.size() - 2] + 5;
	
	//intersection
	int first_start = start_minLen > start_cut ? start_cut : start_minLen;
	int first_end = end_maxLen > end_cut ? end_maxLen : end_cut;
	vector<int> Reverse_cutRange;
	Reverse_cutRange.push_back(first_start);// -----end----------------------start---
	Reverse_cutRange.push_back(first_end);
	for (int i = 0; i < cut.size(); ++i) {
		for (int cutsite = Reverse_cutRange[0]; cutsite >= Reverse_cutRange[1]; --cutsite) {
			for (int site = cutsite; site < cutsite + high_homo_length; ++site) {
				score += ScoreAlignedSite(Aligned_Sequences, site, similar_table);
			}
			if (score > best_score) {
				best_score = score;
		...
		reversecut.push_back(best_site);
		
		// cut[0] = 0, no more reverse cut site.
		if (i == cut.size() - 2) {
			break;
		}

		start_minLen = Find_length_index_backward_all(Aligned_Sequences,best_site, min_length);
		end_maxLen = Find_length_index_backward_one(Aligned_Sequences, best_site, max_length);
		if (start_minLen == -1) {
			//too short
			break;
		}
		start_cut = cut[cut.size() - 2 - i] - 5;//-5bp,for not near to forward cut site.
		end_cut = cut[cut.size() - 3 - i] + 5;

		Reverse_cutRange[0] = start_minLen > start_cut ? start_cut : start_minLen;
		Reverse_cutRange[1] = end_maxLen > end_cut ? end_maxLen : end_cut;


		if (Reverse_cutRange[0] - Reverse_cutRange[1] < 1) {
			warn_flag = 2;
			break;
		}
		//clear
...
int main(int argc, char* argv[]) {
	...
	int warn_flag = FindCutSite(Aligned_Sequences, min_length, max_length, high_homo_length, similar_table, cut, reversecut);
	if (warn_flag == 2) {
		cout << "--------------WARNING---------------" << endl;
		cout << "The Programs exit unexpectedly!" << endl;
		cout << "Try use low homologous options!" << endl;
		cout << "--------------WARNING---------------" << endl;
		string align_error = "mail_dir_file.exe " + dirName + " " + Email + " " + userName;
		char al[100];
		strcpy(al, align_error.c_str());
		system(al);
		exit(-1);
	}
	if (warn_flag == 3) {
		cout << "--------------ERROR---------------" << endl;
		cout << "The Programs exit unexpectedly! " << endl;
		cout << "The input short DNA sequences have very low homology!" << endl;
		cout << "Please design oligos manully!" << endl;
		cout << "--------------ERROR---------------" << endl;
		string align_error = "mail_dir_file.exe " + dirName + " " + Email + " " + userName;
		char al[100];
		strcpy(al, align_error.c_str());
	...
	if (homo_both_side == 0) {
		...
		outoligofile << "The first forward oligo or last reverse oligo will be very short" << endl;
